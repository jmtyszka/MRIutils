function [s, info, errmsg] = parxload2dseq(imnddir)
% [s, info, errmsg] = parxload2dseq(imnddir)
%
% Load a Paravision 2dseq file into Matlab
% Assumes imnd, acqp, subject files are present for this series.
%
% ARGS :
% imnddir = Paravision study directory containing imnd file
% 
% RETURNS :
...
if isequal(parxacqmeth(imnddir),'pvm')
  [info,status] = pvmloadinfo(imnddir);  
else
  [info,status] = parxloadinfo(imnddir);
end
...
filename = fullfile(imnddir,'pdata','1','2dseq');
...
% Catch failures
if fd < 1
  errmsg = sprintf('Problem opening %s',filename);
  return
end
